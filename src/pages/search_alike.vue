...
export default {
  ...
  created(){
    let that=this;
    var planStr = sessionStorage.getItem('alike_plan');
    that.msg = JSON.parse(planStr);
    if (that.msg==''|| that.msg==null) {that.flag = false;}

    window.addEventListener('setItem', ()=> {
    	that.msg = sessionStorage.getItem('alike_plan');
    });
  },
  // watch:{
  //   planStr: function(newVal,oldVal){
  //     console.log(newVal);
  //   }
  // },
  methods:{
      ...
      if(a==""){$("#locate").focus();}
        ...
        this.axios({
          ...
          if(res.data.code == 0){
            ...
            that.msg = res.data.oneCityLineList;
            if (that.msg==''|| that.msg==null) {that.flag = false;}
            that.resetSetItem('alike_plan',JSON.stringify(res.data.oneCityLineList));
            that.$router.push({
              path:'/search_alike',
            });
            $(".loading").css("display","none");

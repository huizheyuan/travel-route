<template lang="html">
  <section class="pc">
    <router-link to="/search_alien" style="margin: 20px;" class="btn">返回所有路线页面</router-link>
    <article class="plan">
      <blockquote>
      ...
      <ul v-for="mDetail,index in msg.startLineInfoBean.shortLines">
          <li v-if="mDetail.shortLineType==1"><!--火车-->
            ...
            <div class="box">
              ...
              <button @click="booking()">购票</button>
            </div>
          </li>
...
export default {
  ...
  created(){
    ...
    let a=this.msg.lineInfoStart;
    let b=this.msg.lineInfoEnd;
    this.search_start = a.substring(-1,a.length-1);
    this.search_end = b.substring(-1,b.length-1);
    let ss=sessionStorage.getItem('start_date').trim();
    this.start_date = ss;

    // console.log(this.search_start);
    // console.log(this.search_end);
  ...
  methods: {
    ...
    // 购票
    // booking('this.search_start','this.search_end','this.start_date'){
    booking(){
  		var urlPath = "https://kyfw.12306.cn/otn/leftTicket/init?linktypeid=dc&fs=" + this.search_start +
  					"&ts=" + this.search_end +
  					"&date=" + this.start_date +
  					"&flag=N,N,Y";
  		window.open (urlPath, '_parent','booking', 'height=1000, width=1000, top=195,left=459, toolbar=no, menubar=no, scrollbars=no, resizable=no,location=no, status=no');
  	},
    //步行导航
    walkMap(mDetail){
      let that=this;
        ...
        if (res.data.code==1) {
          ...
          if (that.comFlag=0){
            for(var i = 0; i < res.data.comments.length;i++){
              that.comments.push(res.data.comments[i])
            }
          }else if (that.comFlag=1){
            that.comments=[];
            for(var i = 0; i < res.data.comments.length;i++){
              that.comments.push(res.data.comments[i])
            }

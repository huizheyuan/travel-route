      ...
      <div class="container-fluid search_box">
        <div class="row">
          <div class="col-lg-3 input-group">
            <input type="text" id="locate" @click="start" class="form-control" placeholder="起始地">
            <span class="input-group-addon">
              <span class="glyphicon glyphicon-map-marker" id="icon" aria-hidden="true"></span>
            </span>
          </div>
          <div class="col-lg-3 input-group">
            <input type="text" id="locate2" @click="end" class="form-control" placeholder="目的地">
            <span class="input-group-addon">
...
<script>
import {SERVERLONG} from '@/config';
import {SERVERSHORT} from '@/config';
...
export default {
  ...
  data: function(){
    return {
      ...
      category: [
        {id: 0, name: '最早到达', sta: false},
        {id: 1, name: '最优价格', sta: false}
      ],
      arr2: [
        [
          //其他
  ...
  created(){
    let that=this;
    var planStr = sessionStorage.getItem('alien_plan');
    that.msg = JSON.parse(planStr);
    if (that.msg==''|| that.msg==null) {that.flag = false;}
    window.addEventListener('setItem', ()=> {
    	that.msg = sessionStorage.getItem('alien_plan');
    });
  },
  methods:{
    // 选框
    ...
    search:function(){
      ...
      if(a==""){$("#locate").focus();}
        ...
        this.axios({
          ...
          that.msg = res.data.lineInfoList;
          if (that.msg==''|| that.msg==null) {that.flag = false;}

          that.resetSetItem('alien_plan',JSON.stringify(res.data.lineInfoList));
          that.$router.push({path:'/search_alien',});
          $(".loading").css("display","none");
          $(".btn").css("display","block");
          sessionStorage.setItem('alien_plan',JSON.stringify(res.data.lineInfoList));
        }).catch(function (err) {
          console.log(err);
          $(".loading").css("display","none");
          $(".btn").css("display","block");
        ...
        this.axios({
          ...
          if(res.data.code == 0){
            ...
            sessionStorage.setItem('alike_plan',JSON.stringify(res.data.oneCityLineList));
            that.$router.push({
              path:'/search_alike',
            });
            $(".loading").css("display","none");
